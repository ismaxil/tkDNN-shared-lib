...
class DetectionNN {
    ...
    public:
         ...
         */
        std::vector<tk::dnn::box> draw(std::vector<cv::Mat>& frames) {

            std::vector<tk::dnn::box> bboxes;
            // std::vector<tk::dnn::box> batch_bboxes;
            ...
            for(int bi=0; bi<frames.size(); ++bi){
                ...
                for(int i=0; i<batchDetected[bi].size(); i++) { 
                    ...
                    // // draw rectangle
                    // std::cout << " YOLO   xmin: "<< x0 << " ymin: "<< y0 << " xmax: " << x1 << " ymax: " << y1 << " width: "<< b.w << " height "<<  b.h << std::endl;
