#ifndef DETECTIONNNBATCHES_H
#define DETECTIONNNBATCHES_H
...
class DetectionNN {
    ...
    public:
         ...
         * @param encryption_key private key used for encrypting the rt model.
         * @param n_classes number of classes for the given dataset.
         * @param n_batches maximum number of batches to use in inference
         * @return true if everything is correct, false otherwise.
         */
        virtual bool init(const std::string& tensor_path, const std::string& encryption_key, const int n_classes=80, const int n_batches=1, const float conf_thresh=0.3) = 0;
         ...
         */
        std::vector<std::vector<tk::dnn::box>> draw(std::vector<cv::Mat>& frames) {

        std::vector<std::vector<tk::dnn::box>> batch_bboxes;
        ...
        for(int bi=0; bi<frames.size(); ++bi){
            ...
            for(int i=0; i<batchDetected[bi].size(); i++) { 
                ...
                batch_bboxes[bi].push_back(bbox);
