...
namespace tk { namespace dnn {
    ...
    tk::dnn::Network* darknetParser(const std::string& cfg_file, const std::string& wgs_path, const std::string& names_file, const std::string& encryption_key) {
...
// Derive key and iv from user-given password
        ...
        std::string decrypted_cfg_data;
        std::ifstream encrypted_cfg(cfg_file, std::ios::binary);
        CryptoPP::FileSource(encrypted_cfg, true, new CryptoPP::AuthenticatedDecryptionFilter(decrypt, new CryptoPP::StringSink(decrypted_cfg_data)));

        // std::string line;
        std::istringstream if_cfg(decrypted_cfg_data);

        if(if_cfg.str().empty()) {
            FatalError("could not open cfg file: " + cfg_file);
        }
