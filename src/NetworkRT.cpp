...
NetworkRT::NetworkRT(Network *net, const char *name, const std::string& encryption_key) {
    ...
    if(!fileExist(name)) {
        ...
        builderActive = true;
        serialize(name,serializedEngineRT, encryption_key);
        delete serializedEngineRT;
#if NV_TENSORRT_MAJOR >= 8
        deserialize(std::string(std::string(name) + ".aes").c_str(), encryption_key);
#endif

#endif
    } else {
        builderActive = false;
        deserialize(name, encryption_key);
    }
...
#if NV_TENSORRT_MAJOR > 5 && NV_TENSORRT_MAJOR < 8
bool NetworkRT::serialize(const char *filename, const std::string& encryption_key) {

    std::ofstream p(std::string(filename) + ".aes", std::ios::binary);
    if (!p) {
    ...
    size_t fsize = ptr->size(); // Get engine file size
    std::stringstream engine_data;
    engine_data.write((char*) ptr->data(), fsize);
    ptr->destroy();
    CryptoPP::SecByteBlock derived(32);
    CryptoPP::PKCS5_PBKDF2_HMAC<CryptoPP::SHA512> kdf; // key derivation function

    kdf.DeriveKey(derived.data(), derived.size(), 0, (CryptoPP::byte*)encryption_key.data(), encryption_key.size(), NULL, 0, 10000);

    // Encrypt TRT engine file and save to disk
    // std::ofstream encrypted_engine_file(encrypted_engine_path, std::ios::binary);

    CryptoPP::EAX<CryptoPP::AES>::Encryption encrypt;
    encrypt.SetKeyWithIV(derived.data(), CryptoPP::AES::DEFAULT_KEYLENGTH, derived.data()+CryptoPP::AES::BLOCKSIZE, CryptoPP::AES::BLOCKSIZE);
    CryptoPP::FileSource(engine_data, true, new CryptoPP::AuthenticatedEncryptionFilter(encrypt, new CryptoPP::FileSink(p)));
    return true;
}
#else
bool NetworkRT::serialize(const char *filename,nvinfer1::IHostMemory *ptr, const std::string& encryption_key){
    std::ofstream p(std::string(filename) + ".aes", std::ios::binary);
    if (!p) {
        FatalError("could not open plan output file");
        return false;
    ...
    std::stringstream engine_data;
    // engine_data.write((char*) ptr->data(), fsize);
    engine_data.write(reinterpret_cast<const char*>(ptr->data()), fsize);
...
bool NetworkRT::deserialize(const char *filename, const std::string& encryption_key) {
    ...
    decrypt.SetKeyWithIV(derived.data(), CryptoPP::AES::DEFAULT_KEYLENGTH, derived.data()+CryptoPP::AES::BLOCKSIZE, CryptoPP::AES::BLOCKSIZE);

    std::string decrypted_engine_data;
    // std::ifstream encrypted_engine_file(encrypted_engine_path, std::ios::binary);
    CryptoPP::FileSource(file, true, new CryptoPP::AuthenticatedDecryptionFilter(decrypt, new CryptoPP::StringSink(decrypted_engine_data)));
    runtimeRT = createInferRuntime(loggerRT);
    size_t size = decrypted_engine_data.size();
    char *gieModelStream = const_cast<char*>(decrypted_engine_data.c_str());
